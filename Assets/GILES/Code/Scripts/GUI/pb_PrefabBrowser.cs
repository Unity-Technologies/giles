...
namespace GILES.Interface
{
	public class pb_PrefabBrowser : MonoBehaviour
	{
		...
		private void createAssetBundleButton(string bundleName){
			...
			pb_PrefabBrowserItemAssetBundlenButton button = icon.AddComponent<pb_PrefabBrowserItemAssetBundlenButton>();
			button.asset = Resources.Load<GameObject> (pb_Config.Default_Folder_Path + "/" + pb_Config.Default_AssetBundle_Thumbnail);
			if (button.asset == null) {
				button.asset = new GameObject ();
			}
			button.asset.name = bundleName;
			button.bundleName = bundleName;
			button.browser = this;
			button.Initialize();
		}
		...
		public void openFolder(string path){
			...
			List<string> fileNames = dMap.getFilesMatch("\\.prefab");

			//If root, display only resource folders
			if (path == "") {
				createHomeIcons ();
			} else //Display Folder
			{
				...
				//Add Back Button
				createFolderButton(dMap.getParentDirectory(),"", "../");
